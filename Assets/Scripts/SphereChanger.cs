...
public class SphereChanger : MonoBehaviour
{
    ...
    public void ChangeSphere(Transform nextSphere)
    {
        ...
        var puerta = GlassDoor.GetComponent<Animator>();
        //Start the fading process
        var f = console.GetComponent<Renderer>().material.name;
        ...
        if (f == "blue emission (Instance)" && pabierta == 0)
        {
            puerta.SetBool("character_nearby", true);
            switchPuerta.GetComponent<LineRenderer>().material = materialActivo;
            pabierta = 1;
        }


    }
    public void ChangeSpheremainroom(Transform nextSphere)
    {
        //Start the fading process
        StartCoroutine(FadeCamera(nextSphere));
    }
    ...
    IEnumerator FadeCamera(Transform nextSphere)
    {
        ...
        if (m_Fader != null)
        {
            ...
            //hand = GameObject.Find("CameraRig");
